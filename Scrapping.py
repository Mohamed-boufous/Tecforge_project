        ...
        with st.container(border=True, key=str(item.get('_id'))):
            ...
            with col_boutons:
                ...
                if st.button("⚙️ Traiter ce Dossier", key=f"process_{item.get('_id')}", use_container_width=True):
                    ...
                    # MODIFIÉ : On passe le lien de téléchargement direct pour le traitement
                    st.session_state.lien_a_traiter = item.get("lien_dossier_direct")
                    st.rerun()
...
def display_process_view(client, model):
    ...
    st.text_input("Lien du dossier à traiter :", value=lien, disabled=True)
